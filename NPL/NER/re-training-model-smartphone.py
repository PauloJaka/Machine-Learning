...
def extract_entities_from_csv(text, row):
    ...
    if pd.notnull(row['model']):
        model_entity = find_entity('MODEL', re.escape(row['model']), text, entities)
        if model_entity:
    ...
    if pd.notnull(row['storage_capacity']):
        storage_entity = find_entity('STORAGE', re.escape(row['storage_capacity']), text, entities)
        if storage_entity:
...
def main():
    ...
    csv_file = "/media/paulo-jaka/Extras/Machine-learning/output_smartphoneslast.csv"  # Caminho do CSV atualizado
    model_dir = "/media/paulo-jaka/Extras/Machine-learning/NPL/NER/trained_models/modelo_ner_celulares"  # Diretório do modelo existente
